...
Defines the centralized, structured logging surface for the backend, establishing Pino as the single logger, request correlation via request ids, environment-aware output formatting, and the complete removal of raw `console.*` logging from application code.
