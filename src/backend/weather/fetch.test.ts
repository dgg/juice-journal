...
describe("fetchWeather", () => {
	it("returns start and end snapshots for two-location forecast", async () => {
		...
		expect(result.start!.temperature).toBe(13.5)
		expect((result.start!.temperature as any).u).toBeUndefined()
		expect(result.end!.temperature).toBe(15.0)
		expect(result.start!.wind.speed).toBe(7.0)
