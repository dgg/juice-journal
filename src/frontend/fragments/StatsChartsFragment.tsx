...
function StatCard({
	...
	const formatted = hasValue ? (value % 1 === 0 ? value.toString() : value.toFixed(1)) : "--"

	const delta = value !== null && prev !== null ? value - prev : null
	const absDelta = delta !== null ? Math.abs(delta) : null
	const formattedDelta =
		absDelta !== null
			? (absDelta % 1 === 0 ? absDelta.toString() : absDelta.toFixed(1))
			: null
		...
		<main class="container">
			<section id="stats-region">
				<div class="period-switcher" role="group">
					{(["week", "month", "year"] as const).map((p) => (
						<button
							...
							hx-swap="outerHTML"
							hx-vals={JSON.stringify({ period: p, yearGranularity: selectedGranularity })}
						>
					...
					<div class="year-granularity" role="group">
						{(["month", "week"] as const).map((g) => (
							<button
								...
								hx-target="#stats-region"
								hx-swap="outerHTML"
								hx-vals={JSON.stringify({ period: "year", yearGranularity: g })}
							>
								{g === "month" ? "Month" : "Week"}
							</button>
						))}
				...
				<div class="stats-period-label">
					...
					{data.vehicle ? (
						<small> — {data.vehicle.description}</small>
					) : null}
				</div>
